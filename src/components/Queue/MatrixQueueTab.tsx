...
const MatrixQueueTab: React.FC = () => {
  ...
  const reload = async () => {
    try {
      const [nextAccounts, nextTasks] = await Promise.all([listSessionAccounts(), listMatrixTasks()]);
      ...
      <Card className="shadow-none bg-default-50">
        <CardBody className="gap-4">
          ...
          {grouped.map(([platformName, items]) => (
            <div key={platformName} className="rounded-xl border border-divider p-3">
              <div className="mb-2 flex items-center justify-between">
                <div className="font-medium">{platformName}</div>
                <span className="text-xs text-default-500">{items.length} 个</span>
              </div>
                ...
                {items.map((account) => (
                  <label key={account.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-lg p-2 hover:bg-default-100">
                    ...
                    <Chip
                      ...
                      {account.sessionStatus === "ready" ? "会话就绪" : "离线"}
                    </Chip>
                  </label>
                ))}
              </div>
          ...
          {!accounts.length && <div className="text-sm text-default-500">账号池为空，请先到“账号池”添加账号。</div>}
        </CardBody>
      </Card>
